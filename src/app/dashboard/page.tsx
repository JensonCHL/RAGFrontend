...
function DashboardPage() {
  ...
  const fetchQdrantData = async () => {
    try {
      ...
      if (data.success) {
        ...
        const companiesWithDocuments: QdrantCompany[] = Object.entries(
          data.data
        ).map(([name, documents]) => ({
          name,
        ...
        setError(
          data.error || "Failed to fetch companies with documents from Qdrant"
        );
      }
    } catch (err) {
      setError(
        "Failed to connect to the Qdrant backend service. Make sure the Flask API is running."
      );
  ...
  const deleteCompany = async (companyName: string) => {
    if (
      !confirm(
        `Are you sure you want to delete all data for company "${companyName}"? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(
          `Successfully deleted company "${companyName}" and all its documents.`
        );
      } else {
  ...
  const deleteDocument = async (companyName: string, documentName: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/proxy/api/companies/${companyName}/documents/${encodeURIComponent(
          documentName
        )}`,
        {
          method: "DELETE",
            ...
            company.name === companyName
              ? {
                  ...
                  documents: Object.fromEntries(
                    Object.entries(company.documents).filter(
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      for (const companyName of selectedCompanies) {
        setDeleting({ company: companyName });
        const response = await fetch(
          `/api/proxy/api/companies/${companyName}`,
          {
            method: "DELETE",
          }
        );
      ...
      for (const { company, document } of selectedDocuments) {
        ...
        const response = await fetch(
          `/api/proxy/api/companies/${company}/documents/${encodeURIComponent(
            document
          )}`,
          {
            method: "DELETE",
            ...
            prev.map((comp) =>
              comp.name === company
                ? {
                    ...comp,
                    documents: Object.fromEntries(
                      Object.entries(comp.documents).filter(
                        ([name]) => name !== document
                      )
                    ),
                  }
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center flex-grow">
                        {isSelecting && (
                          <input
                            ...
                            onChange={() =>
                              toggleSelection(`company:${company.name}`)
                            }
                        ...
                        >
                          <svg
                            className={`w-5 h-5 text-gray-400 mr-3 transform transition-transform duration-200 ${
                              company.isExpanded ? "rotate-90" : ""
                            }`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                          {Object.keys(company.documents).length}{" "}
                          {Object.keys(company.documents).length === 1
                            ? "document"
                            : "documents"}
                        </span>
                        {!isSelecting && (
                    ...
                    <div className="px-6 pb-6">
                      {Object.keys(company.documents).length > 0 ? (
                        <ul className="mt-2 space-y-3">
                          {Object.entries(company.documents).map(
                              ...
                              >
                                {isSelecting && (
                                  <input
                                    ...
                                    checked={
                                      !!selectedItems[
                                        `document:${company.name}:${documentName}`
                                      ]
                                    }
                                    onChange={() =>
                                      toggleSelection(
                                ...
                                <div className="ml-3 flex-grow">
                                  ...
                                  <div className="mt-1 text-xs text-gray-500 space-y-1">
                                    ...
                                    <div className="flex items-center">
                                      ...
                                      <span>
                                        {formatUploadTime(metadata.upload_time)}
                                      </span>
                                    </div>
                                    <div className="flex items-center">
                                      <span className="inline-block w-20">
                                ...
                                {!isSelecting && (
                                  <button
                                    onClick={() =>
                                      deleteDocument(company.name, documentName)
                                    }
                                    disabled={
