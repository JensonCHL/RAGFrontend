...
function DashboardPage() {
  ...
  const deleteCompany = async (companyName: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/companies/${companyName}`, {
        method: 'DELETE',
      });
  ...
  const deleteDocument = async (companyName: string, documentName: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5001/api/companies/${companyName}/documents/${encodeURIComponent(documentName)}`, {
        method: 'DELETE',
      });
  ...
  const bulkDelete = async () => {
    ...
    try {
      ...
      for (const { company, document } of selectedDocuments) {
        ...
        const response = await fetch(`http://localhost:5001/api/companies/${company}/documents/${encodeURIComponent(document)}`, {
          method: 'DELETE',
        });
