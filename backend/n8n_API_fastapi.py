...
if N8N_API_KEY:
    print(
        f"API Key: {N8N_API_KEY[:5]}...{N8N_API_KEY[-5:]}"
    )  # Show first 5 and last 5 chars
    ...
    try:
        ...
        for company, sources in company_documents.items():
            company_data = {
                ...
                "Contract Title": ", ".join(
                    sources
                ),  # Join all sources with comma and space
            }
...
async def get_documents_by_company_name(
    company_name: str, api_key_verified: bool = Depends(verify_api_key)
):
    ...
    try:
        ...
        company_data = [
            item
            for item in documents_data["companies"]
        ...
        if company_data:
            ...
            result = {
                "total_companies": 1,
                "total_contracts": len(
                    company_data[0].get("Contract Title", "").split(", ")
                )
                if company_data[0].get("Contract Title")
                else 0,
...
async def get_document_chunks(
    ...
    try:
        if not document:
            raise HTTPException(
                status_code=400, detail="Missing required parameter: 'document'"
            )
        ...
        search_filter = rest.Filter(
            must=[
                rest.FieldCondition(
                    key="metadata.source", match=rest.MatchValue(value=document)
                )
            ]
        )
        ...
        sorted_points = sorted(
            points, key=lambda p: p.payload.get("metadata", {}).get("page", 0)
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve document chunks: {str(e)}"
        )
